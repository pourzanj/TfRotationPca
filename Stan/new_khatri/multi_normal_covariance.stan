functions {
  ...
  real lambda_jacobian(vector lambda, int p) {
    real lp_ = 0.0;
    for(i in 1:(p-1)) {
      for(j in (i+1):p) {
        lp_ += log(lambda[i]-lambda[j]);
      }
    }
    return(lp_);
  }
...
transformed parameters{
  ...
  // set covariance
  Sigma = Y*diag_matrix(lambdaSq)*Y';
}
model {
  r ~ normal(1.0, 0.1);
  target += lambda_jacobian(lambdaSq,p);
  
  X ~ multi_normal(rep_vector(0.0, p), Sigma);
}
