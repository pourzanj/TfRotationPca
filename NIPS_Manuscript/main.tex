...
\begin{abstract}
Probabilistic PCA (PPCA) is a probabilistic generative model used for finding low-dimensional representations of high-dimensional data. While PPCA is highly flexible in theory, use in practice poses a number of challenges arising from the representation of the orthonormal matrix parameters of the model. These challenges make it difficult in practice to build complex PPCA-like models and conduct fully-Bayesian inference on them using modern algorithms like NUTS and ADVI. To address this challenge, we introduce a geometrically-motivated representation of orthonormal matrices in terms of unconstrained parameters using the Givens Transforms (GT). Our transformed-variable approach, which we refer to as GT-PPCA, enables rapid building and prototyping of PPCA and related models in a probabilistic programming framework like Stan. This opens up the possibility for fully-Bayesian inference on a wide-class of dimensionality reduction models using a variety of different inference algorithms. Our GT-PPCA approach also provides geometric insights into the latent variables allowing us to address important, unresolved issues arising in Bayesian dimensionality reduction, such as tractable hierarchical modeling of subspaces and identifiability. We demonstrate how our GT-PPCA approach can be used in real-world applications by developing a hierarchical subspace model for medical data, as well as an HMM for time-dependent, low-rank modeling of social networks. To facilitate use and adoption by the wider community, provide Stan code for GT-PPCA and our example models.
\end{abstract}
